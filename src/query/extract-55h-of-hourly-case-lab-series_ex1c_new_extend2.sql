...
DROP MATERIALIZED VIEW IF EXISTS case_55h_hourly_labs_ex1c_new_extend2 CASCADE;
CREATE materialized VIEW case_55h_hourly_labs_ex1c_new_extend2 AS
SELECT
  pvt.icustay_id
  , pvt.subject_id
  , pvt.chart_time
...
( -- begin query that extracts the data
  ...
  from cases_hourly_ex1c ch -- was icustays ie (changed it below as well)
    ...
    LEFT JOIN labevents le
      ...
      AND le.charttime BETWEEN (ch.sepsis_onset - interval '96' hour) AND (ch.sepsis_onset+interval '7' hour)
      AND le.ITEMID in
    (
